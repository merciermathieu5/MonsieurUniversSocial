...
def convertir_blocs(texte: str) -> str:
    ...
    return BLOC.sub(remplacer, texte)


def lire_fiche(chemin: Path) -> dict:
    ...
    donnees["html"] = md.convert(convertir_blocs(corps.strip()))
...
def charger_tout(config: dict) -> dict:
    ...
    for cle, meta in config["sections"].items():
        dossier = CONTENU / cle
        fiches = sorted((lire_fiche(p) for p in dossier.glob("*.md")),
